...
function ProfileMenu({ patient, onLogout, onProfilePicChange, loading }) {
  ...
  const [localPic, setLocalPic] = useState(toDataURL(patient?.profilePic) || null);
  ...
  useEffect(() => {
    const newPic = toDataURL(patient?.profilePic);
    if (newPic && newPic !== localPic && !localPic?.startsWith('blob:')) {
      setLocalPic(newPic);
    }
  }, [patient?.profilePic, localPic]);
  ...
  // Handles file change and upload for the profile picture
  ...
  // Maps to the Java UserController's @PatchMapping("/{id}/profile-pic")
  const handleProfilePicUpload = async (file) => {
    ...
    try {
      const response = await fetch(`http://localhost:2002/api/users/${patient.userId}/profile-pic`, {
        method: 'PATCH',
        body: formData,
      });
      ...
      if (onProfilePicChange) {
        // The server returns the updated user object with the new image.
        // We convert the base64 string to a data URL to update the UI.
      ...
      setLocalPic(toDataURL(patient?.profilePic) || null);
  ...
  // Maps to the Java UserController's @PatchMapping("/{id}")
  const handleNameUpdate = async () => {
    setIsUpdating(true);
    try {
      const response = await fetch(`http://localhost:2002/api/users/${patient.userId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username: newName }), // Use 'username' to match the User entity
      });
      ...
      // In a real app, you'd want to refresh the patient data
      handleClose();
    } catch (error) {
  ...
  // This function sends a PATCH request with the new password to the backend
  ...
  const renderNameUpdateForm = () => (
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
        <Button onClick={handleClose} disabled={isUpdating} color="error" startIcon={<CloseIcon />}>
          Cancel
        </Button>
        <Button onClick={handleNameUpdate} disabled={isUpdating} variant="contained" color="primary" startIcon={<CheckIcon />}>
  ...
  const renderPasswordUpdateForm = () => (
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
        ...
        <Button onClick={handlePasswordUpdate} disabled={isUpdating} variant="contained" color="primary" startIcon={<CheckIcon />}>
          {isUpdating ? <CircularProgress size={24} /> : 'Save'}
        </Button>
      </Box>
    </Box>
  ...
  return (
    <>
      <Tooltip title="Open Profile Menu" arrow>
        ...
        >
          {localPic ? (
            <Avatar alt={patient.name} src={localPic} sx={{ width: 44, height: 44, border: `2px solid ${theme.palette.primary.main}` }} />
          ) : (
            <Avatar {...stringAvatar(patient.name)} sx={{ width: 44, height: 44, border: `2px solid ${theme.palette.primary.main}` }} />
          )}
        </IconButton>
      </Tooltip>
        ...
        {isEditingName ? (
          ...
          <Box sx={{ p: 2 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
              {localPic ? (
                <Avatar alt={patient.name} src={localPic} sx={{ width: 72, height: 72, mr: 2, border: `3px solid ${theme.palette.primary.main}` }} />
              ) : (
                <Avatar {...stringAvatar(patient.name)} sx={{ width: 72, height: 72, mr: 2, border: `3px solid ${theme.palette.primary.main}` }} />
              )}
              <Box>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <Typography variant="body2" sx={{ mb: 0.5 }} noWrap>
                <strong>User ID:</strong> {patient.userId || 'N/A'}
              </Typography>
